import type { MaybeCustomPath } from "@demo/types";
import type { NestedPath, HistoricalData, QueryParams, HourlyData } from "@shared/types";
import type { TimeInterval } from "@shared/types/power-hub";
import { customSnakeCase } from "@shared/utils";
...
export const usePollingApi = <T>(
  ...
  ((<Observable<T>>cache[endpoint]) ??= defer(() =>
    ajax<T>({
      url: `${import.meta.env.VITE_API}${toSnakeCase(endpoint.replace("custom:", ""))}`,
...
export const toHistoricalData = <V>({
  time,
  value,
}: HistoricalData<string, V>): HistoricalData<Date, V> => ({
  time: new Date(time),
  value,
});
