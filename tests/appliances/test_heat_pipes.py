...
def test_equal_temps(heat_pipes, simulationtime):
...
def test_differential_temp(heat_pipes, simulationtime):
...
def test_hub_stagnation_temp(hub_heat_pipes, simulationtime):
    ...
    for _ in range(1000):
        pipes_state, outputs = hub_heat_pipes.simulate(
            inputs, pipes_state, None, simulationtime
        )
        temp_diff = (
